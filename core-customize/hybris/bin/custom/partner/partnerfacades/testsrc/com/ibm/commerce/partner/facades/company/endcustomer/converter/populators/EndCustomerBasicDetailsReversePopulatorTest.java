package com.ibm.commerce.partner.facades.company.endcustomer.converter.populators;

import com.ibm.commerce.partner.company.endcustomer.data.IbmPartnerEndCustomerB2BUnitData;
import com.ibm.commerce.partner.core.model.IbmPartnerEndCustomerB2BUnitModel;
import com.ibm.commerce.partner.facades.util.IbmPartnerEndCustomerB2BUnitTestDataGenerator;
import de.hybris.bootstrap.annotations.UnitTest;
import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;
import org.mockito.InjectMocks;
import org.mockito.MockitoAnnotations;

@UnitTest
public class EndCustomerBasicDetailsReversePopulatorTest {

    @InjectMocks
    EndCustomerBasicDetailsReversePopulator endCustomerBasicDetailsReversePopulator;

    IbmPartnerEndCustomerB2BUnitData source;
    IbmPartnerEndCustomerB2BUnitModel target;

    private final static String DCID = "Test Dcid";

    @Before
    public void setUp() {
        MockitoAnnotations.initMocks(this);
        endCustomerBasicDetailsReversePopulator = new EndCustomerBasicDetailsReversePopulator();
        target = new IbmPartnerEndCustomerB2BUnitModel();
        source = IbmPartnerEndCustomerB2BUnitTestDataGenerator.createIbmPartnerEndCustomerB2BUnitData(
            true, null);
        source.setIbmCustomerDCID(DCID);
    }

    @Test
    public void testPopulate() {
        endCustomerBasicDetailsReversePopulator.populate(source, target);
        Assert.assertEquals(DCID, target.getIbmCustomerDCID());
    }
}